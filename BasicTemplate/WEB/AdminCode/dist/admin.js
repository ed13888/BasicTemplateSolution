; layui.extend({ setter: "config" }).define(["setter", "view"], function (e) {
    var a = layui.jquery
        , t = layui.laytpl
        , i = layui.element
        , n = layui.setter
        ...
        , k = C.events = {
            ...
            refresh: function () {
                ...
                var url = route.path.join("/");
                //默认跳转首页
                if (!url) url = layui.setter.entry;
    ...
    r.on("resize", layui.data.resizeSystem),
        e("admin", C),
        layui.link(n.base + "style/admin.css?v=" + (i.v + "-1"), function () {
            var _admin = layui.admin;
            var route = _admin.correctRouter(layui.router().path.join("/"));
            layui.element.render(),
                _admin.screen() < 2 && _admin.sideFlexible();
            //_admin.pageType = "console"
        }, "layuiAdmin");
});
